...
TEST_CASE(focusEffectConfig) {
    NLog::log("{}Testing decoration:focus_effect config parsing", Colors::GREEN);
    ...
    OK(getFromSocket("/eval hl.config({ decoration = { focus_shrink_scale = 0.6 } })"));
    NOK(getFromSocket("/eval hl.config({ decoration = { focus_flash_opacity = 1.5 } })"));
    NOK(getFromSocket("/eval hl.config({ decoration = { focus_shrink_scale = 0.05 } })"));

    OK(getFromSocket("/eval hl.config({ decoration = { focus_effect = 'none' } })"));
    OK(getFromSocket("/eval hl.config({ decoration = { focus_flash_opacity = 0.5 } })"));
    OK(getFromSocket("/eval hl.config({ decoration = { focus_shrink_scale = 0.8 } })"));
}

TEST_CASE(focusEffectAnimations) {
    NLog::log("{}Testing focus effect animation-node registration", Colors::GREEN);
...
TEST_CASE(focusEffectDispatcher) {
    NLog::log("{}Testing hl.dsp.window.focus_effect dispatcher validation", Colors::GREEN);
    ...
    NOK(getFromSocket("/dispatch hl.dsp.window.focus_effect({ effect = 'none' })"));
